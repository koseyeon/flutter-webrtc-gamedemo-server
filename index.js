...
io.on("connection", (socket) => {
  ...
  socket.on("offer", (data) => {
    let { to, from, offerType, offerSDP } = data;

    console.log("[offer] data", { to, from });

    socket.to(to).emit("offer", { from, offerSDP, offerType });
  });
  ...
  socket.on("iceCandidate", (data) => {
    ...
    socket.to(to).emit("iceCandidate", { candidate, sdpMid, sdpMLineIndex, to });
  });

  // close peer connection
  socket.on("disconnectRequest", (data) => {
    const { to } = data;

    console.log("[disconnect] to ", { to });

    if (to !== null) {
      socket.to(to).emit("disconnectResponse");
    }
  });
